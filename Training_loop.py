#putting everything into you have built into one repeating bloack of code, so the network practice actually hundred times instead of writing once Think of it exactly like the toy market game, but now: instead of seeing one toy once, you see all 4 toys, one after another, and you do this whole "see toy → guess → get corrected" routine 1000 times.
...
for epoch in range(epochs):
    total_loss=0
    for i in range(4): #Go through alll 4 examples
        x=X[i]
        actual=Y[i]
        #Forward Pass
        z_hidden=np.dot(w1,x)+b1
